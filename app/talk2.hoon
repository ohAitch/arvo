...
=>  |%                                                  ::  data structures
    ...
    ++  house-5                                         ::
      $:  stories/(map knot story)                      ::  conversations
          ...
          shells/(map client-id shell)                       ::  interaction state
...
++  ra                                                  ::  per transaction
  ...
  ++  sh                                                ::  per console
    ...
    ++  sh-abet
      ...
      =+  zoc=(flop coz)
      |-  ^+  +>+>
      ?~  zoc  +>+>.$(shells (~(put by shells) cid she))
...
::       ~&  sh-fact+[ost.hid sup.hid]
  ...
  ++  ra-sole
    ...
    ^+  +>
    =+  shu=(~(get by shells) cid)
    ?~  shu
      ~|  :+  %ra-console-broken  cid
          ?:((~(has by sup.hid) ost.hid) %lost %unknown)
      !!
    sh-abet:(~(sh-sole sh ~ cid u.shu) act)
  ::
  ...
  ++  ra-console                                        ::  console subscribe
    |=  {her/ship cid/client-id pax/path}
    ^+  +>
    ?:  (~(has by shells) cid)  +>.$ :: REVIEW reprint backlog?
    =+  man=`knot`?~(pax (main her) ?>(?=($~ t.pax) i.pax))
    =+  ^=  she  ^-  shell
        [her man 0 *sole-share ~ [[%& our.hid man] ~ ~] [~ ~] ~ *cabal ~]
    sh-abet:~(sh-peer sh ~ cid she)
  ::
...
++  peer                                                ::  accept subscription
  ...
  ?:  ?=({@ $sole *} pax)
    ...
    ~?  (~(has by shells) cid)  [%talk-peer-replaced cid pax]
    ra-abet:(ra-console:ra src.hid cid t.t.pax)
...
++  poke-talk-command                                   ::  accept command
  ...
  [(welp mos mow) +>.$]
::
++  poke-sole2-action                                ::  accept console
  |=  {cid/client-id act/sole-action}
  ra-abet:(ra-sole:ra cid act)
::
++  diff-talk-report                                    ::
  |=  {way/wire rad/report}
  ^-  (quip move +>)
